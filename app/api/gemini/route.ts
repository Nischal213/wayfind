...
export async function POST(request: NextRequest) {
    ...
    if (!data || error) return NextResponse.json({ error: error?.message || "Something went wrong!" }, { status: 500 })
    ...
    try {
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        valid: { type: "boolean" },
                        action: { type: "string", enum: ["create", "delete", "mixed"] },
                        nodes: {
                            type: "array",
                            items: {
                                type: "object",
                        ...
                        edges: {
                            ...
                            items: {
                                ...
                                required: ["source", "target", "cost"]
                            }
                        },
                        deleteNodes: { type: "array", items: { type: "string" } },
                        deleteEdges: {
                            type: "array",
                            items: {
                                type: "object",
                    ...
                    required: ["valid", "action", "nodes", "edges", "deleteNodes", "deleteEdges"]
                }
            },
            contents: prompt
        })
        ...
        if (isGeminiError(e)) {
            if (e.status === 429) {
                return NextResponse.json({ error: "Gemini rate limit reached, try again later!" }, { status: 429 })
            }
            if (e.status >= 500) {
                return NextResponse.json({ error: "Gemini's servers are having issues, try again later!" }, { status: 503 })
            }
        }

        return NextResponse.json({ error: "Something went wrong generating your response." }, { status: 500 })
    }
}